...
#define    kThemeableBrowserSendMessage @"SendMessage"
...
+ (BOOL)canInitWithRequest:(NSURLRequest *)request
{
    ...
    if ([path containsString:@"http://injection/"] ) {
        return YES;
    } else {
        return NO;
    }
...
- (void)startLoading
{
    ...
    NSString *path = super.request.URL.absoluteString;
    if ([path containsString:@"http://injection/"] ) {
        path = [path stringByReplacingOccurrencesOfString:@"http://injection/file://" withString:@""];
    }else{
        return;
    }
    ...
    //加载本地资源
    NSData *imageData = [self DataWithPath:path];
    [self sendResponseWithData:imageData mimeType:mimeType];
}
...
- (void)sendResponseWithData:(NSData *)data mimeType:(nullable NSString *)mimeType
{
    ...
    header[@"Content-Length"] = [NSString stringWithFormat:@"%ld",data.length];
...
- (void)show:(CDVInvokedUrlCommand*)command withAnimation:(BOOL)animated
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.themeableBrowserViewController != nil) {
            ...
            self.themeableBrowserViewController.view.frame = CGRectMake(0,20,self.themeableBrowserViewController.view.frame.size.width,self.themeableBrowserViewController.view.frame.size.height - 20);
            [self.viewController.view addSubview:self.themeableBrowserViewController.view];
            [self.themeableBrowserViewController didMoveToParentViewController:self];
...
- (void)hide:(CDVInvokedUrlCommand*)command
{
    if (self.themeableBrowserViewController != nil) {
        ...
        [self.themeableBrowserViewController willMoveToParentViewController:nil];
        [self.themeableBrowserViewController.view removeFromSuperview];
        [self.themeableBrowserViewController removeFromParentViewController];
        self.themeableBrowserViewController.view.frame = CGRectMake(0,20,self.themeableBrowserViewController.view.frame.size.width,self.themeableBrowserViewController.view.frame.size.height + 20);
        _isShown = NO;
    }
}
...
- (BOOL)webView:(UIWebView*)theWebView shouldStartLoadWithRequest:(NSURLRequest*)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    if ([[url scheme] isEqualToString:@"gap-iab"]) {
        ...
        [self sendMessage:[url absoluteString]];
...
- (void)sendMessage:(NSString*)param
{
    NSDictionary *event = @{
                            @"type": kThemeableBrowserSendMessage,
                            @"param": param
                            };
    [self emitEvent:event];
}
...
- (id)initWithUserAgent:(NSString*)userAgent prevUserAgent:(NSString*)prevUserAgent browserOptions: (CDVThemeableBrowserOptions*) browserOptions navigationDelete:(CDVThemeableBrowser*) navigationDelegate statusBarStyle:(UIStatusBarStyle) statusBarStyle
{
    ...
    if (self != nil) {
        ...
        // 添加自定义的userAgent
        _userAgent =[userAgent stringByAppendingString:@" themeablebrowser/mslAdp"];
        _prevUserAgent = [prevUserAgent stringByAppendingString:@" themeablebrowser/mslAdp"];
...
- (void)close
{
    //fix bug about close view but video play still
    [self.webView stringByEvaluatingJavaScriptFromString:@"window.location.href='about:blank'"];
